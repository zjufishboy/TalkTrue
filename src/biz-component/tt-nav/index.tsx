  ...
  return (
    <div className="tt-nav-item" onClick={handleClick}>
      <Logo
        ...
        height={20}
        src={NavIcon()}
        color={isNow ? "#33B8EB" : "gray"}
        isSVG
      />
      <span style={{ color: isNow ? "#33B8EB" : "gray" }}>
        {i18n.intl(name)}
      </span>
    </div>
  );
...
export const Nav: React.FC = () => {
  const NavConfig: NavItemProps[] = [
    { name: "PAGE_HOME", url: "/" },
    { name: "PAGE_SEARCH", url: "/search" },
    { name: "PAGE_ABOUT", url: "/about" },
  ];
  return (
    <div className="tt-nav">
      {NavConfig.map((nav) => (
        <NavItem name={nav.name} url={nav.url} key={nav.name} />
      ))}
    </div>
  );
};
